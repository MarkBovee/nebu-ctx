 ...
 recall (search), consolidate (extract session findings), promote (batch hosted promotion), upkeep (recompute hosted lifecycle ranking when supported), triage (preview/apply cleanup candidates when supported), \
            ...
            json!({
                ...
                "properties": {
                    ...
                    "mode": {
                        "type": "string",
                        "description": "Optional triage mode. Use 'apply' to confirm triage changes; default is preview."
                    },
                    "items": {
                        "type": "array",
                        "items": { "type": "object" },
...
pull (receive files shared by other agents), list (show all shared contexts), clear (remove your shared contexts).",
        ...
        tool_def(
            "ctx_cost",
            "Cloud-only analytics surface. Local client no longer serves cost attribution.",
            json!({
                "type": "object",
                "properties": {
                    "action": {
        ...
        tool_def(
            "ctx_gain",
            "Cloud-only analytics surface. Local client no longer serves gain reports.",
            json!({
                "type": "object",
                "properties": {
                    "action": {
        ...
        tool_def(
            "ctx_heatmap",
            "Cloud-only analytics surface. Local client no longer serves heatmaps.",
            json!({
                "type": "object",
                "properties": {
                    "action": {
...
symbol (lookup definition/usages as file::name), impact (blast radius of changes to path), status (index stats).", json!({"type": "object", "properties": {"action": {"type": "string"}, "path": {"type": "string"}, "project_root": {"type": "string"}}, "required": ["action"]})),
        ("ctx_brain", "Server-backed canonical fact memory (Postgres). Actions: store, ingest, recall, forget. Requires server connection.", json!({"type": "object", "properties": {"action": {"type": "string", "enum": ["store", "ingest", "recall", "forget"]}, "key": {"type": "string"}, "value": {"type": "string"}, "query": {"type": "string"}, "kind": {"type": "string"}, "category": {"type": "string"}, "source_type": {"type": "string"}, "source_scope": {"type": "string"}, "promotion_identity": {"type": "string"}, "logical_key": {"type": "string"}, "confidence": {"type": "number"}, "evidence": {"type": "string"}}, "required": ["action"]})),
