...
if command -v claude >/dev/null 2>&1 && command -v nebu-ctx >/dev/null 2>&1; then
    if ! claude mcp list 2>/dev/null | grep -Eq "lean-ctx|nebu-ctx"; then
        LEAN_CTX_QUIET=1 nebu-ctx init --agent claude >/dev/null 2>&1
  fi
fi
"#,
    );
    match std::fs::write(&env_sh, content) {
        Ok(()) => {
            if !matches!(std::env::var("LEAN_CTX_QUIET"), Ok(value) if value.trim() == "1") {
