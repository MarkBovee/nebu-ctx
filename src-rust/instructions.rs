...
• Edit/StrReplace -> native (nebu-ctx=READ only). If Edit needs Read and Read is unavailable, use ctx_edit.
...
\n\
nebu-ctx MCP — MANDATORY tool mapping:\n\
...
• ls/find -> ctx_tree(path, depth)\n\
• Edit/StrReplace -> use native (nebu-ctx only replaces READ, not WRITE)\n\
• Write, Delete, Glob -> use normally\n\
\n\
COMPATIBILITY: nebu-ctx replaces READ operations only. Edit/Write/StrReplace stay native.\n\
