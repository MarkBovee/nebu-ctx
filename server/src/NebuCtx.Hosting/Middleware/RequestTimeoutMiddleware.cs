...
public sealed class RequestTimeoutMiddleware
{
    ...
    public RequestTimeoutMiddleware(RequestDelegate next, ServerOptions options, ILogger<RequestTimeoutMiddleware> logger)
    {
        _next = next;
        _timeout = TimeSpan.FromMilliseconds(options.RequestTimeoutMs);
        _logger = logger;
    }
