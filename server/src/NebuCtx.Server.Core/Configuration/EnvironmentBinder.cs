namespace NebuCtx.Server.Core.Configuration;
...
{
    /// <summary>
    /// Creates a new <see cref="ServerOptions"/> instance populated from environment variables.
    /// </summary>
    public static ServerOptions CreateConfiguredOptions()
    {
        return BindFromEnvironment(new ServerOptions());
    }
    ...
    /// <summary>
    /// Copies all configured values from one options instance to another.
    /// </summary>
    public static void CopyTo(ServerOptions source, ServerOptions target)
    {
        ArgumentNullException.ThrowIfNull(source);
        ArgumentNullException.ThrowIfNull(target);

        target.McpHost = source.McpHost;
        target.McpPort = source.McpPort;
        target.DashboardHost = source.DashboardHost;
        target.DashboardPort = source.DashboardPort;
        ...
        target.MaxRequestsPerSecond = source.MaxRequestsPerSecond;
        target.RateBurst = source.RateBurst;
        target.RequestTimeoutMs = source.RequestTimeoutMs;
        target.TokenFilePath = source.TokenFilePath;
        target.DashboardDisableAuth = source.DashboardDisableAuth;
