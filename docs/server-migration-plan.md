...
>
> Note: the .NET host under `server/` already exists. Treat it as the existing NebuCtx Cloud baseline to extend, not as something to rebuild from scratch. For current product naming and scope decisions, use `docs/nebu-ctx-lean-ctx-realignment-plan.md` as the authoritative plan.
