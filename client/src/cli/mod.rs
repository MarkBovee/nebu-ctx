...
pub fn cmd_session() {
    ...
    for line in &history {
        ...
        if cmd.starts_with("lean-ctx") || cmd.starts_with("nebu-ctx") {
            via_hook += 1;
            total += 1;
        } else {
...
\x1b[2m  Full guide: https://nebu-ctx.com/docs/workflow\x1b[0m"
    );
}
...
pub fn cmd_terse(args: &[String]) {
    ...
    match action {
        ...
        _ => {
            ...
            println!();
            println!("Usage: nebu-ctx terse <off|lite|full|ultra>");
